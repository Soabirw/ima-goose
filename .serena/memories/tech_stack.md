...
- Package manifest exists at repo root; Node package is private `ima-goose` at version `2.6.8`, ESM, requiring Node `>=24`.
...
- Goose provider profiles live in `profiles/*.yaml`; current documented profiles include `chatgpt_codex`, `openai`, `hybrid`, `anthropic`, `claude-acp`, and `sakana`.
